#' Cross-validation for angularlasso
#' 
...
#' 
#' @aliases cv.angularlasso cv.ls cv.logit cv.angular
...
#' be missing.
#' \code{pred.loss = "loss"}.
#' @param \dots other arguments that can be passed to angularlasso.
#' @return an object of class \code{\link{cv.angularlasso}} is returned, which is a
...
#' minimum.}
#' @seealso \code{\link{angularlasso}}, \code{\link{plot.cv.angularlasso}},
...
#' 
#' @export
cv.angularlasso <- function(x, y, group, lambda = NULL, pred.loss = c("misclass", 
    "loss", "L1", "L2", "cosine"), nfolds = 5, foldid, delta, ...) {
    if (missing(pred.loss)) 
        pred.loss <- "default" else pred.loss <- match.arg(pred.loss)
    N <- nrow(x)
    ###Fit the model once to get dimensions etc of output
    y <- drop(y)
    angularlasso.object <- angularlasso(x, y, group, lambda = lambda, ...)
    lambda <- angularlasso.object$lambda
    # predict -> coef
    if (missing(foldid)) 
    ...
    for (i in seq(nfolds)) {
        which <- foldid == i
        y_sub <- y[!which]
        outlist[[i]] <- angularlasso(x = x[!which, , drop = FALSE], y = y_sub, group = group, 
            lambda = lambda, ...)
    }
    ###What to do depends on the pred.loss and the model fit
    fun <- paste("cv", class(angularlasso.object)[[2]], sep = ".")
    ...
    out <- list(lambda = lambda, cvm = cvm, cvsd = cvsd, cvupper = cvm + cvsd, 
        cvlo = cvm - cvsd, name = cvname, angularlasso.fit = gglasso.object)
    lamin <- getmin(lambda, cvm, cvsd)
    obj <- c(out, as.list(lamin))
    class(obj) <- "cv.angularlasso"
    obj
}
...
#' @export
cv.angular <- function(outlist, lambda, x, y, foldid, pred.loss, delta) {
    typenames <- c(cosine = "Cosine loss")
    if (pred.loss == "default") 
        pred.loss <- "cosine"
    predmat <- matrix(NA, length(y), length(lambda))
    nfolds <- max(foldid)
